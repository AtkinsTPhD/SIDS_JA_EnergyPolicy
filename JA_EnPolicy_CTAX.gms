...
*Fuel & Variable Costs for thermal plants
         SUM((plant,thermal),PGG(year,quart,dtype,htype,plant,thermal)*
                 (hrate(plant,thermal)/1000*(FuelPrice(year,thermal)+(CO2FactorTon(thermal)*CTAXrate)$((ord(year)+2016) ge 2019))+VOM(plant,thermal)))
...
$offecho
$ONTEXT
GTEPJA_CTAX.optfile=1 ;
GTEPJA_CTAX.resLim=604000;
$OFFTEXT

Model GTEPJA_CTAX_Transp /
objdef,
...
$offecho
*$ONTEXT
GTEPJA_CTAX_Transp.optfile=1 ;
GTEPJA_CTAX_Transp.resLim=6048000;
*GTEPJA_CTAX_Transp.resLim=388800;

*Model can loop over polcy options
loop(policy$(ord(policy) eq 6),
*CTAXRate = CTAX(policy);

*Here, I manually set carbon tax rates after already having run amny loops
CTAXRate = 63;
Solve GTEPJA_CTAX_Transp using MIP minimizing totcost;
...
CTAXCost(year,policy)$((ord(year)+2016) ge 2019) = discf(year)*(SUM((quart,dtype,htype),Mquart(quart)*Mday(dtype)*mth(htype)*(
                             SUM((plant,thermal)$PlantType(plant,thermal),PGG.l(year,quart,dtype,htype,plant,thermal)*
                 (hrate(plant,thermal)/1000*CO2factorTon(thermal)*CTAXRate)))))/1000;

TotalCTAXCost(policy) = sum(year, CTAXCost(year,policy));

NetSystemCost(year,policy) = TotCostY(year,policy) -  (CTAXCost(year,policy)/1000);
...
*-----------------------------------
*$Ontext
Execute_unload "Output_CarbonTax.gdx" stotcost,totcosty, LCOE,  opcost, Sinvcostplant, Sinvcosttline, Sbuildplant, SbuildTline,Annual_Generation,GenPortfolio,GenPortfolioShare,capfactor,Emissions,TotalEmission,CTAXCost,TotalCTAXCost, NetSystemCost,TotalNetSystemCost
Execute "gdxxrw.exe Output_CarbonTax.gdx par=stotcost rng=STotalCost!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=TotCosty rng=yrlyTotCost!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=LCOE rng=LCOE_centskWh!"
*Execute "gdxxrw.exe Output_CarbonTax.gdx Par=UEGWh rng=Unserved_EnergyGWh!"
*Execute "gdxxrw.exe Output_CarbonTax.gdx Par=UEshare rng=Percent_Unmet_Demand!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=opcost rng=OpCost_USDM!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=Sinvcostplant rng=InvCostPlant!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=Sinvcosttline rng=InvCostTline!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=SBuildPlant rng=BuildPlant!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=SBuildTline rng=BuildTline!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=Annual_Generation rng=Generation_GWh!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=GenPortfolio rng=GenPortfolio_GWh!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=GenPortfolioShare rng=GenPortfolioShare!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=CapFactor rng=SCapFator!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=Emissions rng=yrlyEmissions!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=TotalEmission rng=TotalEmission!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=CTAXCost rng=yrlyCTAXCost!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=TotalCTAXCost rng=TotalCTAXCost!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=NetSystemCost rng=NetSystemCost!"
Execute "gdxxrw.exe Output_CarbonTax.gdx par=TotalNetSystemCost rng=TotalNetSystemCost!"
*$OFFTEXT
