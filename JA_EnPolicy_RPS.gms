...
Parameter TotalDemand(year) Total Demand MWh;
...
RPS_cons(year)$((ord(year)+2016) ge 2030)..SUM((quart,dtype,htype,plant,retecnb)$PlantType(Plant,retecnb),
...
$offecho
*$ONTEXT
GTEPJA_RPS.optfile=1 ;
GTEPJA_RPS.resLim=6048000;
*$OFFTEXT

loop(policy$(ord(policy) eq 2),
RETarget = RPS(policy);
Solve GTEPJA_RPS using MIP minimizing totcost;


*Using Transportation Model instead
$Ontext
Model GTEPJA_RPS_Transp /
objdef,
...
GTEPJA_PS_Transp.resLim=6048000;
$Offtext
...
*-----------------------------------
*$Ontext
Execute_unload "Output_RPS.gdx" stotcost,totcosty, LCOE, opcost, Sinvcostplant, Sinvcosttline, SBuildPlant, SBuildTline,Annual_Generation,GenPortfolio,GenPortfolioShare,capfactor, Emissions, TotalEmission
Execute "gdxxrw.exe Output_RPS.gdx par=stotcost rng=STotalCost!"
Execute "gdxxrw.exe Output_RPS.gdx par=TotCosty rng=yrlyTotCost!"
Execute "gdxxrw.exe Output_RPS.gdx par=LCOE rng=LCOE_centskWh!"
Execute "gdxxrw.exe Output_RPS.gdx var=u.l rng=U!"
Execute "gdxxrw.exe Output_RPS.gdx par=opcost rng=OpCost_USDM!"
Execute "gdxxrw.exe Output_RPS.gdx par=Sinvcostplant rng=InvCostPlant!"
Execute "gdxxrw.exe Output_RPS.gdx par=Sinvcosttline rng=InvCostTline!"
Execute "gdxxrw.exe Output_RPS.gdx par=SBuildPlant rng=BuildPlant!"
Execute "gdxxrw.exe Output_RPS.gdx par=SBuildTline rng=BuildTline!"
Execute "gdxxrw.exe Output_RPS.gdx par=Annual_Generation rng=Generation_GWh!"
Execute "gdxxrw.exe Output_RPS.gdx par=GenPortfolio rng=GenPortfolio_GWh!"
Execute "gdxxrw.exe Output_RPS.gdx par=GenPortfolioShare rng=GenPortfolioShare!"
Execute "gdxxrw.exe Output_RPS.gdx par=CapFactor rng=SCapFator!"
Execute "gdxxrw.exe Output_RPS.gdx par=Emissions rng=yrlyEmissions!"
Execute "gdxxrw.exe Output_RPS.gdx par=TotalEmission rng=TotalEmission!"
*$OFFTEXT
