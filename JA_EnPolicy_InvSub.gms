...
Parameters
 ...
 Emissions(year,policy) Metric Ton of CO2 emitted per year  (discounted)
 TotalEmission(policy) Metric Ton of CO2 produced over planning horizon (discounted)
 ...
 InvSubCost(year,policy) Annual cost of investment subsidy in millions of dollars
 TotalInvSubCost(policy) Total Cost of investment subsidty in millions of dollars over planning horizon
...
*Variable Cost for Renewable plants excluding biomass plants
          +SUM((plant,retecnb),PGG(year,quart,dtype,htype,plant,retecnb)*VOM(plant,retecnb))
...
FCostdef(year)..FixedCost(year) =E= SUM((EPlant,tec)$PlantType(Eplant,tec),FOM(year,EPlant,tec)*AvailableEPlant(year,EPlant,tec))
                                 +SUM((Cplant,tec)$PlantType(Cplant,tec),FOM(year,CPlant,tec)*SUM(y$(ord(y) le ord(year)),BuildPlant(y,cplant,tec)));
*$ontext
InvCostPdef(year)..InvCostPlant(year) =E= (SUM((Cplant,thermal)$PlantType(Cplant,Thermal),OCCg(Cplant,thermal)*SUM(y$(ord(y) le ord(year)),BuildPlant(y,cplant,thermal))
                                          *crfg(Cplant,thermal))
                                          + SUM(Cplant,OCCg(Cplant,'hydro')*SUM(y$(ord(y) le ord(year)),BuildPlant(y,cplant,'hydro'))
                                          *crfg(Cplant,'hydro'))
                                          + SUM(Cplant,(1-InvSubRate)*OCCg(Cplant,'solar')*SUM(y$(ord(y) le ord(year)),BuildPlant(y,cplant,'solar'))
                                          *crfg(Cplant,'solar'))
                                          + SUM((Cplant),(1-InvSubRate)*OCCg(Cplant,'wind')*SUM(y$(ord(y) le ord(year)),BuildPlant(y,cplant,'wind'))
                                          *crfg(Cplant,'wind')))
                                         *discf(year);
...
$offecho
$ONTEXT
GTEPJA_InvSubsidy.optfile=1 ;
GTEPJA_InvSubsidy.resLim=6040000;
$OFFTEXT

Model GTEPJA_InvSub_Transp /
objdef,
...
$offecho
*$ONTEXT
GTEPJA_InvSub_Transp.optfile=1 ;
GTEPJA_InvSub_Transp.resLim=6048000;

*Looping over policy range
loop(policy$(ord(policy) eq 3),
*InvSubRate = InvSubsidy(policy);

*Manually setting investment subsidy rate having already done many loops.
InvSubRate = 0.35;
Solve GTEPJA_InvSub_Transp using MIP minimizing totcost;
...
*Variable Cost for Renewable plants excluding biomass plants
          +SUM((plant,retecnb)$PlantType(plant,retecnb),PGG.l(year,quart,dtype,htype,plant,retecnb)*VOM(plant,retecnb))
...
Emissions(year,policy) = discf(year)*SUM((quart,dtype,htype),Mquart(quart)*Mday(dtype)*mth(htype)*(
                             SUM((plant,thermal)$PlantType(plant,thermal),PGG.l(year,quart,dtype,htype,plant,thermal)*
                 (hrate(plant,thermal)/1000*CO2factorTon(thermal)))));

TotalEmission(policy) = sum(year,Emissions(year,policy));

InvSubCost(year,policy)$((ord(year)+2016) ge 2019)= (SUM(Cplant,InvSubRate*OCCg(Cplant,'solar')*SUM(y$(ord(y) le ord(year)),BuildPlant.l(y,cplant,'solar'))
                                          *crfg(Cplant,'solar'))
                                          + SUM((Cplant),InvSubRate*OCCg(Cplant,'wind')*SUM(y$(ord(y) le ord(year)),BuildPlant.l(y,cplant,'wind'))
                                          *crfg(Cplant,'wind')))
                                         *discf(year);

TotalInvSubCost(policy) = sum(year, InvSubCost(year,policy));

NetSystemCost(year,policy) = TotCostY(year,policy) +  InvSubCost(year,policy);
...
*-----------------------------------
*Output for Investment Subsidy
*-----------------------------------
*$Ontext
Execute_unload "Output_InvestmentSubsidy.gdx" stotcost,totcosty, LCOE,  opcost, Sinvcostplant, Sinvcosttline, Sbuildplant, SbuildTline,Annual_Generation,GenPortfolio,GenPortfolioShare,capfactor,Emissions,TotalEmission,InvSubCost,TotalInvSubCost, NetSystemCost,TotalNetSystemCost
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=stotcost rng=STotalCost!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=TotCosty rng=yrlyTotCost!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=LCOE rng=LCOE_centskWh!"
*Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx Par=UEGWh rng=Unserved_EnergyGWh!"
*Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx Par=UEshare rng=Percent_Unmet_Demand!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=opcost rng=OpCost_USDM!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=Sinvcostplant rng=InvCostPlant!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=Sinvcosttline rng=InvCostTline!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=SBuildPlant rng=BuildPlant!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=SBuildTline rng=BuildTline!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=Annual_Generation rng=Generation_GWh!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=GenPortfolio rng=GenPortfolio_GWh!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=GenPortfolioShare rng=GenPortfolioShare!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=CapFactor rng=SCapFator!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=Emissions rng=yrlyEmissions!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=TotalEmission rng=TotalEmission!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=InvSubCost rng=yrlyInvSubCost!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=TotalInvSubCost rng=TotalInvCost!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=NetSystemCost rng=NetSystemCost!"
Execute "gdxxrw.exe Output_InvestmentSubsidy.gdx par=TotalNetSystemCost rng=TotalNetSystemCost!"
*$OFFTEXT
